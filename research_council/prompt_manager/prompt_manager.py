...
class PromptManager:
    ...
    def validate_prompt(self, config: dict):
        """
        Validate that the prompt config has all required fields and correct types.
        Raises ValueError if invalid.
        """
        required_fields = ['agent_name', 'version', 'role', 'description', 'core_objectives']
        for field in required_fields:
            if field not in config:
                raise ValueError(f'Missing required field: {field}')
        # Ensure core_objectives is always a list
        if not isinstance(config['core_objectives'], list):
            config['core_objectives'] = [config['core_objectives']]
        # Add more type checks as needed

    def save_prompt(self, agent_name: str, config: Dict[str, Any], create_version: bool = True) -> str:
        self.validate_prompt(config)
        """
    ...
    def fork_prompt(self, source_agent: str, new_agent: str, author: str = None) -> str:
        """
        Create a new prompt by forking an existing one, resetting version and optionally setting author.
        """
        config = self.load_prompt(source_agent)
        config['agent_name'] = new_agent
        config['version'] = 'v1.0'
        if author:
            config['author'] = author
        return self.save_prompt(new_agent, config, create_version=False)

    def set_prompt_status(self, agent_name: str, status: str):
        """
        Set the lifecycle status (active, deprecated, experimental) for a prompt.
        """
        config = self.load_prompt(agent_name)
        config['status'] = status
        self.save_prompt(agent_name, config, create_version=True)

    def get_prompts_by_status(self, status: str) -> list:
        """
        Get a list of agent names with a given status.
        """
        return [a for a in self.get_available_agents() if self.load_prompt(a).get('status') == status]

    def log_session(self, agent_name, prompt_version, input_data, output_data):
        """
        Log an agent session for auditability and reproducibility.
        """
        log_entry = {
            'timestamp': datetime.now().isoformat(),
            'agent_name': agent_name,
        ...
        log_file = self.prompt_dir / 'session_log.json'
        if log_file.exists():
    ...
    def chain_agents(self, agent_sequence: list, initial_input: dict):
        """
        Chain multiple agents together, passing output as input, and log each step.
        """
        data = initial_input
        for agent in agent_sequence:
            prompt = self.load_prompt(agent)
            # Placeholder for actual agent call (e.g., LLM or function)
            agent_output = {'result': f"Output from {agent}"}  # Replace with real call
            self.log_session(agent, prompt['version'], data, agent_output)
            data = agent_output
        return data
    ...
    def increment_usage(self, agent_name: str):
        ...
        """
        usage_file = self.prompt_dir / 'usage_stats.json'
    ...
    def get_usage(self, agent_name: str) -> int:
        """
        Get the usage count for a prompt.
        """
        usage_file = self.prompt_dir / 'usage_stats.json'
        if usage_file.exists():
            with open(usage_file, 'r') as f:
                stats = json.load(f)
            return stats.get(agent_name, 0)
        return 0
